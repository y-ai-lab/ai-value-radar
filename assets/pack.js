(() => {
  ...
  function render(markdown) {
    ...
    for(const line of lines){
      ...
      const heading=line.match(/^(#{1,3})\s+(.+)$/); if(heading){closeList(); const level=Math.min(3,heading[1].length); const node=text(`h${level}`,heading[2]); node.appendChild(inline(heading[2])); node.textContent=""; node.appendChild(inline(heading[2])); fragment.appendChild(node); continue;}
    ...
    closeList(); closePre(); main.replaceChildren(fragment);
  }
