...
## AI VALUE RADARと同じ定期実行方式

`.github/workflows/edgex-breakout-scan.yml` を有効にすると、Public GitHubの標準GitHub-hosted runnerが5分ごとに起動し、EdgeXの全取引可能コントラクトを確認して終了します。GPTやPCを起動したままにする必要はありません。
...
1. このフォルダの内容をPublic GitHubリポジトリへ配置する
...
4. `Actions` タブで `EdgeX breakout scan` を選び、`Run workflow` で手動実行して確認する
5. 問題がなければ、以後は5分ごとの自動実行に任せる
